...
int API::getSize(const std::string &path, uint32_t &size) {
    if (isDirectory(path)) {
        Json *j; if (ensureIndexReady(path)<0 || Cache::instance().acquire(path,j)<0) return -1;
        size = (*j).value("size", 0);
    } else {
        auto pos = path.find_last_of('/');
        std::string dir = path.substr(0,pos);
        std::string fn  = path.substr(pos+1);
        Json *j; if (ensureIndexReady(dir)<0 || Cache::instance().acquire(dir,j)<0) return -1;
        for (auto &rec:*j) if (rec["fn"].get<std::string>()==fn) {size = rec["sz"].get<uint32_t>(); return 0;}
        return -2;
    }
    return 0;
}
...
int API::getDuration(const std::string &path, uint32_t &dur) {
    if (isDirectory(path)) {
        ...
        std::string fn  = path.substr(pos+1);
        Json *j; if (ensureIndexReady(dir)<0 || Cache::instance().acquire(dir,j)<0) return -1;
        for (auto &rec:*j) if (rec["fn"].get<std::string>()==fn) {dur = rec["aud"]["dur"].get<uint32_t>(); return 0;}
        return -2;
    }
    return 0;
}
...
int API::getRecord(const std::string &filePath, Json &record) {
    ...
    Json *j; if (ensureIndexReady(dir)<0 || Cache::instance().acquire(dir,j)<0) return -1;
    for (auto &rec:*j) if (rec["fn"].get<std::string>()==fn) {record = rec; return 0;}
    return -2;
}
